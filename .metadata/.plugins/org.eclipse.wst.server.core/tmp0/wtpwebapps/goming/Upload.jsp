...
<body>
	...
	<main>
      ...
      <!-- hidden -->
			<input type="hidden" id="userGender" name="userGender" value=selectedGender>
			<input type="hidden" id="style" name="style" value=selectedStyle>
			<input type="hidden" id="season" name="season" value= selectedSeason>
	...
	document.addEventListener('DOMContentLoaded', function() {
 	    ...
 	    document.getElemnetById('faceCheck').value = "OFF"; // 기본 모자이크 값은 OFF로 설정
 	    
 	// UI에도 반영 (버튼에 checked 클래스 추가)
 	    document.querySelector('#gender-list .checked').classList.remove('checked');
 	    document.querySelector('#style-list .checked').classList.remove('checked');
 	    document.querySelector('#season-list .checked').classList.remove('checked');
 	    document.querySelector('#face-mosaic-toggle .checked').classList.remove('checked');
 	    
 	    // 초기값에 해당하는 버튼에 checked 클래스 추가
 	    document.querySelector('#gender-list button[value="F"]').classList.add('checked');
 	    document.querySelector('#style-list button[value="미니멀"]').classList.add('checked');
 	    document.querySelector('#season-list button[value="여름"]').classList.add('checked');
 	});
    ...
    document.getElementById('submit-btn').addEventListener('click', function(event) {
        ...
        document.getElementBtId('season').value = selectedSeason;
