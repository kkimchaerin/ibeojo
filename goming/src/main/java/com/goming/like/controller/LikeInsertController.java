...
public class LikeInsertController extends HttpServlet
{
	...
	{
		System.out.println("LikeInsertController : 두포스트 들어감");
		...
		System.out.println("LikeInsertController : " + post_img);

		// DAO를 사용하여 좋아요 갯수 가져오기
		LikeDAO dao = new LikeDAO();
		int likeCount = dao.weatherSelectAllCount(post_img);
		System.out.println("LikeCountController : " + likeCount);

		// 결과를 JSON 객체로 변환하여 응답
		JSONObject jsonResponse = new JSONObject();
		jsonResponse.put("likeCount", likeCount);
		System.out.println("LikeInsertController : " + jsonResponse.toString());
