...
window.onload = function() {
    ...
    user_pw = document.querySelector('input#user_pw'); // id가 'user_id'인 input 요소
    user_pw_check = document.querySelector('input#user_pw_check');
   elSuccessMessage = document.querySelector('.success-message'); // 클래스가 'success-message'인 요소
    ...
    // user_id 입력 요소가 존재하는지 확인 후 이벤트 핸들러 추가
    if (user_pw_check) {
        user_pw_check.addEventListener('input', function() {
            // 다시 한 번 요소가 존재하는지 확인
            console.log(user_pw.value);
         console.log(user_pw_check.value);
            if (user_pw && user_pw_check && elSuccessMessage && elFailureMessage) {
            
                if (user_pw_check.value.length !== 0) {
               if(user_pw.value === user_pw_check.value){
                       elSuccessMessage.classList.remove('hide'); // 사용할 수 있는 아이디입니다 메시지 표시
                       elFailureMessage.classList.add('hide'); // 실패 메시지 숨김
               }else{
                  elSuccessMessage.classList.add('hide');
                       elFailureMessage.classList.remove('hide');
               }
                } //  if (user_pw_check.value.length !== 0) 종료
            } // if (user_pw && user_pw_check && elSuccessMessage && elFailureMessage) 종료
        }); // addEventListener 종료
    } // if(user_pw_check) 종료
};